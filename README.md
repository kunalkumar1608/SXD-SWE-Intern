...
**Answer** : To test whether a given "problem pattern" exists in the backend, I have created a python script which connects to a mongoDB instance (intially had empty data). Now, for our requirement we have created a unique key for each problem based on user inputs. Our key stores - Minimum/Maximum flag, Objective function coefficients and Constraints Coefficients. Then, for each key, we will generate the optimal parameters (which acts as a value) using our algorithm used in Question 1 and Question 2. Next, we will query for the given key in our database, which should retrieve values as optimal parameters. If this value exists, then we don't do anything but if it doesn't then we will store a key-value pair in our database for future reference. The integration test script is added as a file name `sxd_question3.py` in this github repository along with sample json files containing data and database. 
